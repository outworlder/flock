...
(define-syntax with-validations
  (syntax-rules ()
    ([_ validation-ok validation-fail ((field (validation params ...) ...) ...) ]
     (begin
       (let ([validation-results
         ...
         (if (> (length (filter string? validation-results)) 0)
             (validation-fail validation-results) ;Validation failed
             (validation-ok)))))))

 
;; Each validation function takes exactly one param
(define (validate-presence value #!key (message " is required"))
  (if (string? value)
      #t
      message))
